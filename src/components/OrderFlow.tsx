...
const VIETNAM_PROVINCES = [
  'An Giang', 'B Ra - Vng Tu', 'Bc Liu', 'Bc Giang', 'Bc Kn', 'Bc Ninh',
  'Bn Tre', 'Bnh Dng', 'Bnh nh', 'Bnh Phc', 'Bnh Thun', 'C Mau',
  'Cao Bng', 'Cn Th', ' Nng', 'k Lk', 'k Nng', 'in Bin',
  'ng Nai', 'ng Thp', 'Gia Lai', 'H Giang', 'H Nam', 'H Ni',
  'H Tnh', 'Hi Dng', 'Hi Phng', 'Hu Giang', 'Ha Bnh', 'TP. H Ch Minh',
  'Hng Yn', 'Khnh Ha', 'Kin Giang', 'Kon Tum', 'Lai Chu', 'Lng Sn',
  'Lo Cai', 'Lm ng', 'Long An', 'Nam nh', 'Ngh An', 'Ninh Bnh',
  'Ninh Thun', 'Ph Th', 'Ph Yn', 'Qung Bnh', 'Qung Nam', 'Qung Ngi',
  'Qung Ninh', 'Qung Tr', 'Sc Trng', 'Sn La', 'Ty Ninh', 'Thi Bnh',
  'Thi Nguyn', 'Thanh Ha', 'Tha Thin Hu', 'Tin Giang', 'Tr Vinh',
  'Tuyn Quang', 'Vnh Long', 'Vnh Phc', 'Yn Bi',
];
...
const products = [
  {
    ...
    extraPageCost: 6000, // 6.000 / trang = 12.000 / tờ (Giấy C150)
  ...
  },
  {
    ...
    nameVi: 'Sách ảnh Bìa Cứng M Phẳng',
    sizes: [
      { label: '20x20cm', value: '20x20' as const, price: 399000 },
    ],
    pagesLimit: 14,
    pagesLabel: '14 trang = 7 tờ (cả bìa)',
    paperType: 'Bìa carton cứng cáng mờ, hiu ứng m phẳng liền mạch khi m 2 trang i din',
    extraPageCost: 15000, // 15.000 / trang = 30.000 / tờ
...
export function OrderFlow({ user, book, onBack, onComplete }: OrderFlowProps) {
  ...
  const validatePhone = (phone: string): boolean => {
    ...
    if (!/^0\d{9}$/.test(phone)) {
      setPhoneError('S in thoi phi gm 10 ch s v bt u bng 0');
      setPhoneError('S in thoại phải gm 10 chữ s và bắt ầu bằng 0');
      return false;
    }
    setPhoneError('');
    return true;
  ...
  const handleShippingSubmit = (e: React.FormEvent) => {
    ...
    if (isBelowMinimum) {
      if (compatibleProducts.length > 0) {
        toast.error(
          `Không ủ ${currentProduct.pagesLimit} trang ti thiu cho ${currentProduct.nameVi}. Vui lòng chọn: ${compatibleProducts.map(p => p.nameVi).join(' hoặc ')}.`,
          { duration: 6000 }
        );
      } else {
        toast.error(
          `Cần ti thiu ${currentProduct.pagesLimit} trang. Vui lòng quay lại bưc chọn trang  thêm ít nhất ${currentProduct.pagesLimit - selectedPageCount} trang nữa.`,
          { duration: 6000 }
        );
      }
      return;
    ...
    if (!shippingInfo.fullName.trim()) {
      toast.error('Vui lng nhp h v tn.');
      return;
    }
    if (!shippingInfo.phone.trim()) {
      toast.error('Vui lng nhp s in thoi.');
      return;
    }
    if (!validatePhone(shippingInfo.phone)) {
      toast.error(phoneError || 'S in thoi khng hp l.');
      return;
    }
    if (!shippingInfo.email.trim()) {
      toast.error('Vui lng nhp email.');
      return;
    }
    if (!shippingInfo.city) {
      toast.error('Vui lng chn tnh/thnh ph.');
      return;
    }
    if (!shippingInfo.district.trim()) {
      toast.error('Vui lng nhp qun/huyn.');
      return;
    }
    if (!shippingInfo.address.trim()) {
      toast.error('Vui lng nhp a ch chi tit.');
      toast.error('Vui lòng nhập họ và tên.');
      return;
    }
    if (!shippingInfo.phone.trim()) {
      toast.error('Vui lòng nhập s in thoại.');
      return;
    }
    if (!validatePhone(shippingInfo.phone)) {
      toast.error(phoneError || 'S in thoại không hợp l.');
      return;
    }
    if (!shippingInfo.email.trim()) {
    ...
    if (!shippingInfo.city) {
      toast.error('Vui lòng chọn tnh/thành ph.');
      return;
    }
    if (!shippingInfo.district.trim()) {
      toast.error('Vui lòng nhập quận/huyn.');
      return;
    }
    if (!shippingInfo.address.trim()) {
      toast.error('Vui lòng nhập a ch chi tiết.');
      return;
    }
  ...
  const handlePdfFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'application/pdf' && !file.name.toLowerCase().endsWith('.pdf')) {
      toast.error('Vui lòng chọn file PDF hợp l.');
      // Reset input
    ...
    if (file.size > MAX_PDF_SIZE) {
      toast.error(`File quá ln (${(file.size / 1024 / 1024).toFixed(1)}MB). Vui lòng chọn file PDF dưi 500MB.`);
      if (pdfInputRef.current) pdfInputRef.current.value = '';
      return;
    }
  ...
  const handlePaymentSubmit = async () => {
    try {
      setLoading(true);
      console.log('[OrderFlow] 1️ Bắt ầu gửi ơn hàng...');
      ...
      console.log('[OrderFlow] 2️ Design pages:', designPages.length, 'items');

      // CH gửi tên file PDF, KHNG gửi ni dung  tránh Connection Reset (100MB+ payload)
      const pdfFileName = pdfFile ? pdfFile.name : null;
      const pdfFileSize = pdfFile ? `${(pdfFile.size / 1024 / 1024).toFixed(1)}MB` : null;
      console.log('[OrderFlow] 3️ PDF file:', pdfFileName, pdfFileSize || '(không có)');
      ...
      const orderData = {
        ...
        note: [
          shippingInfo.notes,
          pdfFileName ? `[PDF ính kèm: ${pdfFileName} - ${pdfFileSize}]` : null,
        ].filter(Boolean).join(' | '),
        ...
        pdfFileData: null, // Không gửi binary data  tránh payload quá ln
      };
      console.log('[OrderFlow] 4️ OrderData prepared, designPages:', designPages.length);

      // Kim tra JSON.stringify không li trưc khi gửi
      try {
        const testJson = JSON.stringify(orderData);
        console.log('[OrderFlow] 5️ JSON.stringify OK, size:', (testJson.length / 1024).toFixed(0), 'KB');
      } catch (jsonErr) {
        console.error('[OrderFlow] 5️ JSON.stringify FAILED:', jsonErr);
        throw new Error('Dữ liu ơn hàng quá ln hoặc chứa ni dung không hợp l. Vui lòng thử lại.');
      }

      console.log('[OrderFlow] 6️ Gọi API placeOrder...');
      const response = await orderApi.placeOrder(userId, orderData);
      console.log('[OrderFlow] 7️ API thành công:', response);

      if (pdfFile) {
        toast.info('⏳ Đang tải file PDF thiết kế lên h thng...');
        await orderApi.uploadPdf(response.id, userId, pdfFile);
        console.log('[OrderFlow] 8️ Tải file PDF thành công');
      }
      ...
      // Trích xuất message li cụ th  hin th cho user
      let errorDetail = '';
      if (err?.message) {
      ...
      toast.error(
        `Không th gửi ơn hàng.${errorDetail ? `\nLi: ${errorDetail}` : ''}\nKim tra: Backend ã chạy chưa? (port 8080)`,
        { duration: 10000 }
      );
      // Không chuyn sang confirmation  giữ user  bưc payment  thử lại
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="lg:col-span-2">
            ...
            {step === 'shipping' && (
              <form onSubmit={handleShippingSubmit} className="space-y-6">
                {/* 1. Chọn loại sách & chất liu giấy */}
                <div className="rounded-2xl p-6 space-y-6 animate-in fade-in duration-300" style={{ background: 'white', border: '1.5px solid #DDD8D0' }}>
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold" style={{ color: '#000000' }}>
                      Chọn loại sách & chất liu giấy
                    </h2>
                  </div>
                  <p className="text-xs -mt-4 text-[#7A6F66]">
                    Vui lòng chọn 1 trong 3 loại photobook cao cấp dưi ây:
                  </p>
                    ...
                    <div className="flex items-start gap-3 p-4 rounded-xl border-2 border-amber-200 bg-amber-50">
                      ...
                      <div>
                        <p className="text-sm font-bold" style={{ color: '#92400e' }}>
                          Không ủ s trang ti thiu cho {currentProduct.nameVi}
                        </p>
                        <p className="text-xs mt-0.5" style={{ color: '#a16207' }}>
                          Bạn ã chọn <span className="font-bold">{selectedPageCount} trang</span>, nhưng {currentProduct.nameVi} yêu cầu ti thiu <span className="font-bold">{currentProduct.pagesLimit} trang</span>.
                          {compatibleProducts.length > 0 ? (
                            ...
                            <span> Vui lòng quay lại bưc chọn trang  chọn thêm ít nhất {currentProduct.pagesLimit - selectedPageCount} trang nữa.</span>
                          )}
                        </p>
                      </div>
                    </div>
                              ...
                              <p className="flex items-center gap-1.5 font-medium">
                                ...
                                {isProdBelowMin && (
                                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-100 text-amber-700 border border-amber-200">
                                    ...
                                    Cần ti thiu {prod.pagesLimit} trang
                                  </span>
                                )}
                              </p>
                          ...
                          <div className="mt-4 pt-3 border-t border-[#EDE9E3] space-y-3">
                            ...
                            <div>
                              <p className="text-[9px] font-bold text-[#9B9088] uppercase tracking-wider mb-1 flex items-center gap-1">
                                <Ruler className="w-2.5 h-2.5" /> Kích thưc:
                              </p>
                  ...
                  <div className="pt-6 border-t border-[#EDE9E3] space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-[#7A6F66]">
                          S trang bạn ã chọn  bưc trưc. Ti thiu {currentProduct.pagesLimit} trang ã bao gm trong giá cơ bản.
                        </p>
                      </div>
                    ...
                    <div className="p-3.5 rounded-xl bg-[#F5F2EE] text-xs text-[#7A6F66] flex flex-col gap-1.5 animate-in fade-in duration-300">
                      <p>
                         S trang mặc nh i kèm: <span className="font-semibold text-[#000000]">{currentProduct.pagesLimit} trang</span> (ã bao gm trong giá cơ bản).
                      </p>
                      {selectedPageCount > currentProduct.pagesLimit ? (
                        <p className="text-[#10b981] font-semibold">
                           Bạn ang in thêm: {(selectedPageCount - currentProduct.pagesLimit)} trang ({Math.ceil((selectedPageCount - currentProduct.pagesLimit) / 2)} tờ). Phụ phí: +{pagePrice.toLocaleString('vi-VN')} .
                          {currentProduct.id !== 'layflat' ? ' (12.000 /tờ C150)' : ' (30.000 /tờ Lay-flat)'}
                        </p>
                      ) : (
                        <p> S trang trong gii hạn tiêu chuẩn, không phát sinh phụ phí.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl p-6" style={{ background: 'white', border: '1.5px solid #DDD8D0' }}>
                  ...
                  <p className="text-xs mb-4" style={{ color: '#7A6F66' }}>
                    Tải lên file PDF bạn ã xuất từ bưc chọn trang. File sẽ ược gửi kèm ơn hàng  in ấn chính xác.
                  </p>
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-bold" style={{ color: '#000000' }}>
                            Nhấn  tải lên file PDF
                          </p>
                          <p className="text-xs mt-0.5" style={{ color: '#9B9088' }}>
                            H trợ file PDF · Ti a 500MB
                          </p>
                        </div>
                      </div>
                    </button>
                ...
                <div className="rounded-2xl p-6" style={{ background: 'white', border: '1.5px solid #DDD8D0' }}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>H v tn *</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>S in thoi *</label>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>S in thoại *</label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>Tnh/Thnh ph *</label>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>Tnh/Thành ph *</label>
                      <select
                        ...
                        style={{ border: '1.5px solid #DDD8D0', color: '#000000', background: '#FAFAF8' }}
                      >
                        <option value="">Chn tnh/thnh ph</option>
                        <option value="">Chọn tnh/thành ph</option>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>Quận/Huyn *</label>
                      <input
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-medium mb-1.5" style={{ color: '#7A6F66' }}>Đa ch chi tiết *</label>
                      <textarea
                        required value={shippingInfo.address}
                        onChange={e => setShippingInfo({ ...shippingInfo, address: e.target.value })}
                        rows={3} placeholder="S nhà, tên ường..."
                ...
                >
                  {isBelowMinimum
                    ? `Cần ti thiu ${currentProduct.pagesLimit} trang (hin có: ${selectedPageCount})`
                    : 'Tiếp tục thanh toán'}
                </button>
              </form>
            )}
            ...
            {step === 'payment' && (
              <div className="space-y-6">
                <div className="rounded-2xl p-6" style={{ background: 'white', border: '1.5px solid #DDD8D0' }}>
                  ...
                  <div className="space-y-3">
                    {[
                      { method: 'full', title: 'Thanh toán trưc', sub: 'Thanh toán trưc 100% giá tr ơn hàng qua VietQR' },
                      { method: 'deposit', title: 'Đặt cọc 50% hàng', sub: 'Đặt cọc trưc 50%, thanh toán 50% còn lại khi nhận hàng' },
                ...
                <div className="flex gap-3">
                  ...
                  >
                    ...
                    {loading ? 'Đang xử lý...' : 'Xác nhận ặt hàng'}
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6 animate-in fade-in duration-500">
                <div className="rounded-2xl p-8 text-center" style={{ background: 'white', border: '1.5px solid #DDD8D0' }}>
                  ...
                  <p className="mb-6 text-sm" style={{ color: '#7A6F66' }}>
                    Cảm ơn bạn ã tin tưng DearMemories. Cun sách của bạn ang ược xử lý thiết kế và in ấn!
                  </p>
                  ...
                  <div className="p-6 rounded-2xl border border-[#DDD8D0] bg-[#FAFAF8] mb-6 text-center space-y-4 max-w-sm mx-auto">
                    <p className="font-bold text-sm text-[#000000]">
                      {paymentMethod === 'deposit' ? 'Quét mã  chuyn khoản ặt cọc 50%:' : 'Quét mã  thanh toán 100%:'}
                    </p>
                    ...
                    <div className="space-y-1 text-xs text-[#7A6F66]">
                      <p>
                        S tiền cần chuyn: <span className="font-extrabold text-base text-emerald-600">{totalPrice.toLocaleString('vi-VN')} </span>
                      </p>
                      <p>
                        Ni dung chuyn khoản: <span className="font-mono font-bold text-[#000000]">BK{orderId || 'DEARBOOK'}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 rounded-2xl border-2 border-amber-200 bg-amber-50 text-left space-y-3 mb-6 max-w-md mx-auto">
                    <div className="flex items-center gap-2 text-amber-800">
                      ...
                      <p className="font-bold text-sm">
                        Hoàn tất chuyn khoản ơn hàng
                      </p>
                    </div>
                    <p className="text-xs text-amber-700 leading-relaxed">
                      Đ xác nhận chuyn khoản thành công và ẩy nhanh sản xuất, bạn vui lòng iền thông tin và tải ảnh hóa ơn giao dch tại Form xác nhận sau:
                    </p>
                    <a
                    ...
                    >
                      Click iền Form xác nhận chuyn khoản 
                    </a>
                  </div>
                  ...
                  >
                    <p className="font-semibold text-sm" style={{ color: '#000000' }}> Thông tin ơn hàng:</p>
                    <div className="space-y-1.5 text-sm" style={{ color: '#7A6F66' }}>
                      <p> Mã ơn: <span className="font-mono font-bold text-[#000000]">#{orderId || `BK${Date.now()}`}</span></p>
                      <p> Loại photobook: <span className="font-semibold text-[#000000]">{currentProduct.nameVi}</span></p>
                      <p> Kích thưc: <span className="font-semibold text-[#000000]">{selectedSize}</span></p>
                      <p> S trang: <span className="font-semibold text-[#000000]">{selectedPageCount} trang</span></p>
                      <p> Hình thức thanh toán: <span className="font-bold text-orange-600">{paymentMethod === 'deposit' ? 'Đặt cọc trưc 50%' : 'Thanh toán trưc 100%'}</span></p>
                      ...
                      <p> S tiền cần chuyn khoản: <span className="font-bold text-[#000000]">{totalPrice.toLocaleString('vi-VN')} </span></p>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="space-y-4 mb-5">
                ...
                <div className="space-y-2 text-sm pt-3" style={{ borderTop: '1px solid #EDE9E3' }}>
                  ...
                  <div className="flex justify-between">
                    <span style={{ color: '#7A6F66' }}>Phí vận chuyn</span>
                ...
                <div className="pt-3" style={{ borderTop: '1px solid #EDE9E3' }}>
                  {paymentMethod === 'deposit' && (
                    <div className="flex justify-between text-xs text-[#7A6F66] mb-1.5 animate-in fade-in duration-300">
                      <span>Đã bt 50% ặt cọc:</span>
                      <span className="font-semibold text-rose-500">-( {(totalOriginal * 0.5).toLocaleString('vi-VN')}  )</span>
                    </div>
                  )}
                  <div className="flex justify-between font-bold">
                    <span style={{ color: '#000000' }}>{paymentMethod === 'deposit' ? 'Đặt cọc 50%' : 'Tng cng'}</span>
              ...
              >
                ...
                <p style={{ color: '#7A6F66' }}> Giao hàng úng hẹn</p>
              </div>
            </div>
          </div>
        </div>
