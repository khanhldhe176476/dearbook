...
const LOCAL_TEMPLATES = [
  {
    ...
    name: 'K nim dng hnh yu du',
    description: 'Mt du n ring, lu gi thanh xun. Thit k ti gin, hin i v tinh t.',
  ...
  },
  {
    ...
    name: 'Gi li thanh xun',
    description: 'Lu gi nhng khonh khc khng th qun. Ph hp lm qu tng bn b, k nim.',
...
export function MyBooksLibraryPortfolio({ user, onLogout, onCreateNew, onEditBook, onBackToHome }: MyBooksLibraryPortfolioProps) {
  ...
  const displayTemplates = supabaseTemplates.filter(tpl => 
    ...
    tpl.name !== 'Phong cch Vintage' &&
    tpl.name !== 'Phong cch Hin i' &&
    tpl.name !== 'Phong cch Ti gin'
  );
  ...
  const loadBooks = async () => {
    setLoading(true);
    try {
      // Primary: c t IndexedDB
      let booksSource: BookData[] = [];
      if (isIndexedDBAvailable()) {
        booksSource = await getAllBooks();
      }
      // Fallback: localStorage nu IndexedDB trng hoc khng kh dng
      if (booksSource.length === 0) {
        booksSource = getBooksSync();
      }

      // ng b vi backend API (non-blocking)
      try {
        ...
        // Backend khng kh dng, dng local data
        setBooks(booksSource);
      }
      setError(null);
    } catch (err) {
      console.error('Failed to load books:', err);
      setError('Khng th ti sch. Hin th t b nh cc b.');
  ...
  const handleDuplicate = async (book: BookData) => {
    ...
    loadBooks();
    toast.success('  thm sch mu vo th vin!', { description: `"${duplicated.title}" sn sng chnh sa`, duration: 3000 });
  ...
  const handleDeleteConfirm = async () => {
    ...
    toast.success('  xa sch thnh cng', { duration: 2000 });
  };
  ...
  const themeData = {
    love:       { name: 'Tnh yu', emoji: '', grad: 'from-rose-400 to-pink-500',     badge: 'bg-rose-50 text-rose-600 border-rose-100'   },
    romantic:   { name: 'Tnh yu', emoji: '', grad: 'from-rose-400 to-pink-500',     badge: 'bg-rose-50 text-rose-600 border-rose-100'   },
    family:     { name: 'Gia nh', emoji: '', grad: 'from-sky-400 to-blue-500',     badge: 'bg-sky-50 text-sky-600 border-sky-100'       },
    birthday:   { name: 'Sinh nht', emoji: '', grad: 'from-amber-400 to-orange-500', badge: 'bg-amber-50 text-amber-600 border-amber-100'  },
    friendship: { name: 'Tnh bn', emoji: '', grad: 'from-emerald-400 to-teal-500', badge: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
    travel:     { name: 'Du lch',   emoji: '', grad: 'from-cyan-400 to-blue-500',     badge: 'bg-cyan-50 text-cyan-600 border-cyan-100'     },
    wedding:    { name: 'm ci',  emoji: '', grad: 'from-purple-400 to-indigo-500', badge: 'bg-purple-50 text-purple-600 border-purple-100' },
    kids:       { name: 'Tr em',    emoji: '', grad: 'from-yellow-300 to-amber-500',  badge: 'bg-yellow-50 text-yellow-600 border-yellow-100' },
  };

  const formatDate = (dateString: string) =>
    new Date(dateString).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });

  // Ly theme chnh xc t templateId (u tin book.theme, fallback suy t templateId)
  ...
  // Ly nh preview u tin t sch (nu c)
  ...
  const handleRename = async (book: BookData, newTitle: string) => {
    ...
    try {
      ...
      loadBooks();
      toast.success('  i tn sch');
    } catch (err) {
      console.error('Rename failed:', err);
      toast.error('Khng th i tn sch');
    }
  };
      ...
      >
        ...
        <div className="relative h-52 flex-shrink-0 overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-4">
            ...
            <h3 className="text-sm font-bold text-white drop-shadow-lg line-clamp-2 leading-snug text-center z-10 max-w-[90%]">
              {book.title || <span className="opacity-60 italic">Cha t tn</span>}
            </h3>
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center gap-2.5 z-30
            ...
            <ActionBtn onClick={() => onEditBook(book)} title="Chnh sa" icon={<Edit className="w-4 h-4" />} primary />
            <ActionBtn onClick={() => handleDuplicate(book)} title="Nhn bn" icon={<Copy className="w-4 h-4" />} />
            <ActionBtn onClick={() => handleDeleteClick(book.id, book.title || 'Sch')} title="Xa" icon={<Trash2 className="w-4 h-4" />} danger />
          </div>
        </div>
        ...
        <div className="flex flex-col flex-1 p-4 gap-2.5">
          ...
          {isRenaming ? (
            <div className="flex items-center gap-1.5">
              <input
                ...
                autoFocus
              />
              <button onClick={onRenameConfirm} className="p-1.5 rounded-lg bg-green-500 text-white hover:bg-green-600 transition-colors" title="Lu">
                <Check className="w-3.5 h-3.5" />
              </button>
              <button onClick={onRenameCancel} className="p-1.5 rounded-lg bg-gray-200 text-gray-500 hover:bg-gray-300 transition-colors" title="Hy">
            ...
            <div className="flex items-start gap-1.5 group/title">
              <p
                ...
                title="Click  i tn"
              >
                {book.title || <span style={{ color: '#ccc', fontStyle: 'italic' }}>Cha t tn</span>}
              </p>
              <button
                onClick={() => { setRenameValue(book.title || ''); setIsRenaming(true); }}
          ...
          <div className="flex items-center gap-1.5 text-[11px] font-medium mt-auto" style={{ color: '#999' }}>
            ...
            <span>To {formatDate(book.createdAt)}</span>
            {book.updatedAt !== book.createdAt && (
              <span className="text-[10px]" style={{ color: '#ccc' }}>
                 Cp nht {formatDate(book.updatedAt)}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col flex-1 p-6 gap-4">
          <div>
            ...
            <p className="text-[14px] leading-relaxed line-clamp-2" style={{ color: '#666', minHeight: '2.75rem' }}>
              {tpl.description || 'Mu sch thit k cao cp, lu gi nhng k nim v gi.'}
            </p>
          </div>
    ...
    <div className="min-h-screen" style={{ background: '#faf8f5' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-10">
        ...
        <div className="space-y-1">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight" style={{ color: '#111', lineHeight: 1.2 }}>
            Xin cho, {user.name.split(' ').slice(-1)[0]}! 
          </h2>
          <div className="flex items-center gap-3">
            <p className="text-base sm:text-lg" style={{ color: '#999' }}>
              {books.length > 0
                ? `Bn ang c ${books.length} cun sch trong th vin`
                : 'Bt u to cun sch u tin ca bn'}
            </p>
        ...
        >
          ...
          <div className="relative flex items-center gap-5">
            ...
            <div>
              <p className="text-xl sm:text-2xl font-bold text-white leading-tight">To cun sch mi</p>
              <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.55)' }}>
                Thit k qu tng  ngha ch trong vi pht
              </p>
            </div>
            {/* Arrow */}
        ...
        >
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            ...
            >
              <option value="all">Tt c ch </option>
              <option value="love"> Tnh yu</option>
              <option value="family"> Gia nh</option>
              <option value="birthday"> Sinh nht</option>
              <option value="friendship"> Tnh bn</option>
            </select>
            ...
            >
              <option value="recent">Mi nht</option>
              <option value="oldest">C nht</option>
              <option value="name">Tn A-Z</option>
              <option value="theme">Ch </option>
            </select>
            ...
            <div className="flex gap-1 p-1 rounded-xl" style={{ background: '#f0ede8' }}>
              {(['grid', 'masonry', 'list'] as ViewMode[]).map(mode => (
                <button
                  ...
                  title={mode === 'grid' ? 'Li u' : mode === 'masonry' ? 'Masonry' : 'Danh sch'}
      ...
      <section className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold leading-tight" style={{ color: '#111' }}>Mu Thit K C Nhn Ha</h3>
            <p className="text-sm mt-0.5" style={{ color: '#aaa' }}>
              Cc mu thit k qu tng c o, c c nhn ha trn vn dnh cho bn
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          <div className="overflow-hidden rounded-3xl shadow-lg border border-[#eeece9] bg-white p-2">
            <img src={templateHenryTran} alt="K nim dng hnh yu du" className="w-full h-auto object-cover rounded-2xl" />
          </div>
          <div className="overflow-hidden rounded-3xl shadow-lg border border-[#eeece9] bg-white p-2">
            <img src={templateYouthArchive} alt="Gi li thanh xun" className="w-full h-auto object-cover rounded-2xl" />
          </div>
        </div>
      </section>
        ...
        <section ref={userBooksSectionRef} className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold" style={{ color: '#111' }}>Sch Ca Bn</h3>
                <p className="text-sm mt-0.5" style={{ color: '#aaa' }}>
                  {filteredBooks.length} cun{filteredBooks.length !== books.length ? ` / ${books.length} tng` : ''}
                </p>
              </div>
            </div>
          </div>
          ...
          {filteredBooks.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h4 className="text-xl font-bold mb-2" style={{ color: '#222' }}>
                {searchQuery || filterTheme !== 'all' ? 'Khng tm thy sch no' : 'Cha c cun sch no'}
              </h4>
              <p className="text-sm mb-6" style={{ color: '#aaa' }}>
                {searchQuery || filterTheme !== 'all'
                  ? 'Th thay i b lc hoc t kha tm kim'
                  : 'Bt u hnh trnh to sch c nhn ha ca bn'}
              </p>
              {!searchQuery && filterTheme === 'all' && (
                <button
                  onClick={handleCreateNewClick}
                ...
                >
                  <Plus className="w-4 h-4" /> To sch mi
                </button>
              )}
            </div>
          ) : (
